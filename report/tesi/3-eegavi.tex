...
The selected backbone for the Video embeddings is ViViT\footnote{ViViT: google/vivit-b-16x2-kinetics400}\cite{arnab2021vivitvideovisiontransformer}.
...
\paragraph{Audio}
All audio, when present, is turned into a mono signal and resampled to match the required $fs=16kHz$ by WavLM\footnote{WavLM: microsoft/wavlm-base}\cite{Chen_2022}.
...
For this reason, we transcribe speech using \textit{Whisper}\footnote{Whisper: openai/whisper-medium} by OpenAI\cite{radford2022robustspeechrecognitionlargescale}.
...
The Text processing pipeline uses MiniLM\footnote{sentence-transformers/all-MiniLM-L6-v2}\cite{wang2020minilmdeepselfattentiondistillation}
...
As ECG foundation model, we opted for the ECG-FM\cite{mckeen2025ecgfmopenelectrocardiogramfoundation} which is reported to perform when the data channels are incomplete.
...
We adopt the \textit{SigLIP}\cite{zhai2023sigmoidlosslanguageimage} loss for training, a loss function that is inspired by InfoNCE.
