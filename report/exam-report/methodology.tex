...
\begin{itemize}
    ...
    \item \textbf{DEAP}: \textit{Database for Emotion Analysis} is a dataset composed by data of 32 participants.
...
\begin{table}
    ...
    \end{tabular}
    \caption{
        Repartition of the subjects in the three data splits for training EEGAVI.
    }
...
Thi should allow the model to force a stronger cross subject generalizaiton.
...
\begin{itemize}
    ...
    In EEG it is often associated with high activity as scares/epilepsy or motion artifacts, therefore its contribution is moderated in the scoring process.
...
Pre-processing is done for both \textit{EEGAVI} and \textit{VATE} with each their own pipelines.
...
Their time unit has been selected to be 4 seconds.
This is not necessary for \textit{pivot} whose time unit is of 1 second.
...
The resulting vector acts as key-value in the \textit{cross attention} process while the EEG branch is the query.
This way the conditioning is only directed by EEG data.

Being the model masked on all of its steps, to enure stronger generalization capabilities, sample-modality level dropout is performed.
...
We adopted for training the \textit{SigLIP} loss, a loss function that is inspired by InfoNCE.
...
Standard values for the technique were initially used (momentum $m=0.99$, queue size $q=1024$) but will be in future work fine-tuned.
...
Mixing data is generally a good practice but in the context of EEG it can act as limitation.
...
Inspired by other applications like ANTLM instead of taking single position of the time control we opt for a dynamic training on the setting.
...
this controlled by a cosine ramp schedule.
